...
/**
 * Card — flat surface, 1px hairline border, no shadow, no ring.
 * 6px radius max. Depth comes from the surface ladder, not decoration.
 */
...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card relative flex flex-col gap-4 overflow-hidden rounded-md border border-border bg-card py-4 text-sm text-card-foreground",
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "font-heading text-[0.9rem] leading-tight font-semibold tracking-[-0.005em] text-foreground group-data-[size=sm]/card:text-[0.8rem]",
        className
      )}
      {...props}
    />
